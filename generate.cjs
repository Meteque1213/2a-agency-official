...
if (fs.existsSync('whitepaper.html')) { fs.copyFileSync('whitepaper.html', path.join(outputDir, 'whitepaper.html')); }
...
registry.forEach(brand => {
    ...
    (entity.ownership || []).forEach(o => {
        ownershipHtml += `<div class="mb-3 pb-2 border-b border-black/5"><p class="text-[10px] opacity-40 uppercase">${o.entity}</p><p class="text-xs font-bold">${o.stake} Stake / ${o.voting_rights} Voting</p></div>`;
    });

    // 2. Génération du bloc Strategie & Risques
    let risksHtml = "";
    ...
    (brand.events || []).forEach(e => {
        eventsHtml += `<div class="mb-4"><p class="text-[#065f46] text-[10px] font-black">${e.year} | ${e.type.toUpperCase()}</p><p class="text-xs normal-case font-normal leading-tight">${escapeHtml(e.description)}</p></div>`;
    });
    ...
    // --- TEMPLATE DU RAPPORT D'AUDIT COMPLET ---
    const htmlContent = `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>2A AGENCY | ${escapeHtml(rawName)} Audit</title>
...
<body class="bg-white text-black p-6 md:p-12 italic uppercase font-bold tracking-tight">
    ...
    <main class="max-w-6xl mx-auto">
        <header class="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16 border-b-4 border-black pb-12">
            <div class="md:col-span-2">
                <p class="text-[#065f46] text-[14px] mb-2 tracking-[0.3em] font-black">${escapeHtml(entity.category || "Luxury Portfolio")}</p>
                <h1 class="text-6xl md:text-8xl font-black tracking-tighter text-[#065f46] uppercase leading-none mb-6">${escapeHtml(rawName)}</h1>
                <div class="flex gap-8 opacity-60 text-[10px]">
                    <p>SOR_ID: ${sorId}</p>
                    <p>CEO: ${governance.chairman_ceo?.name || "N/A"}</p>
                    <p>HQ: ${governance.structure || "N/A"}</p>
                </div>
            </div>
            <div class="bg-gray-50 p-8 border border-black/10 text-right flex flex-col justify-center">
                <p class="text-[11px] opacity-40 mb-1 font-normal tracking-widest">AI IQ INDEX</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-12">
            <div class="space-y-10">
                <section>
                    <h3 class="text-[11px] text-[#065f46] mb-6 tracking-widest border-b border-[#065f46]/20 pb-2">01. FINANCIALS</h3>
                    <div class="mb-6">
                        <p class="text-[10px] opacity-40 font-normal">REVENUE</p>
                        <p class="text-3xl font-black italic">${financials.annual_revenue?.value || "N/A"} <span class="text-xs opacity-50">${financials.annual_revenue?.currency || ""}</span></p>
                    </div>
                    <div class="mb-6">
                        <p class="text-[10px] opacity-40 font-normal">MARKET CAP</p>
                        <p class="text-3xl font-black italic">${financials.market_cap?.value || "N/A"} <span class="text-xs opacity-50">${financials.market_cap?.currency || ""}</span></p>
                    </div>
                </section>
                <section class="bg-emerald-50 p-4 border border-[#065f46]/20">
                    <h3 class="text-[11px] text-[#065f46] mb-4 tracking-widest">02. OWNERSHIP</h3>
                    ${ownershipHtml || "Direct control."}
                </section>
            </div>

            <div class="md:col-span-2 space-y-12">
                <section>
                    <h3 class="text-[11px] text-[#065f46] mb-4 tracking-widest border-b border-[#065f46]/20 pb-2">03. CULTURAL MOAT</h3>
                    <p class="text-xl font-medium normal-case font-normal leading-relaxed text-black/80 italic">${escapeHtml(moat)}</p>
                </section>
                <section>
                    <h3 class="text-[11px] text-[#065f46] mb-4 tracking-widest border-b border-[#065f46]/20 pb-2">04. CORE STRATEGY</h3>
                ...
                <section class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <h3 class="text-[11px] text-red-600 mb-4 tracking-widest uppercase italic">Strategic Risks</h3>
                        ${risksHtml || "Low exposure."}
                    </div>
                    <div>
                        <h3 class="text-[11px] text-[#065f46] mb-4 tracking-widest uppercase italic">Key Events</h3>
                        ${eventsHtml || "No recent audit log."}
                    </div>
                </section>
            </div>
            ...
            <div class="bg-black text-white p-6 flex flex-col justify-between">
                <div>
                    <h3 class="text-[11px] text-[#10b981] mb-6 tracking-widest">05. PROOF</h3>
                    <p class="text-[10px] opacity-60 mb-1 font-normal uppercase">CONFIDENCE SCORE</p>
                    <p class="text-5xl font-black italic mb-8">${(brand.proof?.confidence_score * 100).toFixed(0)}%</p>
                    
                    <p class="text-[10px] opacity-60 mb-2 font-normal uppercase tracking-widest">VERIFIED SOURCES</p>
                    ${(brand.proof?.sources || []).map(s => `<p class="text-[9px] mb-2 border-l border-[#10b981] pl-2 opacity-80 uppercase">${s.name}</p>`).join('')}
                </div>
                <div class="pt-6 border-t border-white/20">
                    <p class="text-[9px] font-normal normal-case leading-tight opacity-50">Data anchored on Base (L2) blockchain via 2A Notarization Protocol.</p>
                </div>
            </div>
        </div>
    </main>
    ...
    <tr class="border-b border-black/5 hover:bg-emerald-50 transition-all cursor-pointer group" onclick="window.location.href='${fileName}'">
        ...
        <td class="p-6 opacity-40 text-[10px] font-mono tracking-tighter text-black font-mono">${sorId}</td>
...
<body class="bg-white text-black p-6 md:p-20 italic uppercase font-bold">
    ...
    <main><table class="w-full text-left uppercase"><thead class="opacity-30 text-[9px] tracking-widest border-b border-black/10 font-normal text-black"><tr><th class="p-6">ENTITY NAME</th><th class="p-6">AI_IQ</th><th class="p-6">SOR_ID</th><th class="p-6 text-right uppercase">STATUS</th></tr></thead><tbody>${tableRows}</tbody></table></main>
</body>
</html>`;

fs.writeFileSync(path.join(outputDir, 'index.html'), indexHtml);
console.log(`✅ SUCCESS: Dashboard-style audits generated with all JSON fields.`);